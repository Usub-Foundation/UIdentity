...
| `BearerAuthMiddleware` | `include/uidentity/keycloak/auth/bearer_auth_middleware.hpp` | Authenticates `Authorization: Bearer` requests and stores a `RequestContext`. |
...
For APIs that use bearer headers, `BearerAuthMiddleware` validates the token and writes `RequestContext` into `request.user_data`.
